...
/*   Updated: 2024/10/05 22:33:53 by jcameira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parameter_expansion_str_len(t_minishell *msh, t_final_cmd_table *table,
	char *content)
{
	int	real_len;
	int	quotes[2];
...
char	*expand_parameter(t_minishell *msh, t_final_cmd_table *table,
	char *content, int len)
{
	...
	init_needed_variables(content, &quotes, &index, &contents);
	contents[1] = new_content;
	while (contents[0][++index[0]])
	{
		if (contents[0][index[0]] == '\'' || contents[0][index[0]] == '"')
			check_parameter_quotes(contents[0], &new_content, index, quotes);
		else if (contents[0][index[0]] == '$'
			&& quotes[D] && contents[0][index[0] + 1] == '"')
			contents[1][++index[1]] = contents[0][index[0]];
		else if (contents[0][index[0]] == '$' && contents[0][index[0] + 1]
			&& contents[0][index[0] + 1] != ' ' && !quotes[S])
			contents[1] = add_expanded_parameter(msh, table, contents, &index);
		else
			contents[1][++index[1]] = contents[0][index[0]];
	}
	contents[1][++index[1]] = '\0';
	return (free(contents[0]), contents[1]);
}
