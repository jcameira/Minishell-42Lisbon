...
/*   Updated: 2024/10/05 23:59:22 by jcameira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (final_cmd_table->ambiguous_redirect)
	{
		*status = 1;
		return (ft_putstr_fd(ERROR_PREFIX, 2),
			...
			ft_putstr_fd(AMBIGUOUS_REDIRECT, 2), 0);
	}
...
int	get_pipeline_size(t_final_cmd_table *final_cmd_table)
{
	...
	size = 1;
	tmp = final_cmd_table;
	while (tmp->next_symbol == S_PIPE && tmp->next
		&& tmp->next->subshell_level == tmp->subshell_level)
	{
		tmp = tmp->next;
		size++;
	}
...
void	check_if_pipefd_needed(t_execution_info **info)
{
	...
	{
		(*info)->tmp_table->outfile_fd = \
		(*info)->descending_subshell_pipe[WRITE];
